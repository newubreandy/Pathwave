...
def build_dev_checklist():
    ...
    items = [
        # ───────── 인증/회원가입 ─────────
        ('인증/회원가입', '이전 완료', 'PR #1~#5, #28, #45, #55',
         '이메일 가입·로그인, 5종 소셜 로그인(Google/Apple/Facebook/Kakao/Naver), 동의 단계, 미성년자 보호, 회원 탈퇴 흐름',
         '✅', '코드 완료, 실 키는 서비스 가입 후'),
        ('인증/회원가입', 'Phase 1', 'P4',
         'dev 인증 우회 제거 + 실 로그인폼 복구 + /forgot-password 정식 구현',
         '✅', 'BE 2 API + provider 페이지'),
        ('인증/회원가입', '남은', '실 키 검증',
         'Firebase·카카오·네이버 실 키로 통합 테스트 (서비스 가입 후)',
         '⬜', '외부 서비스 활성화 후'),

        # ───────── 매장 정보 ─────────
        ('매장 정보', '이전 완료', 'PR #2, #14, #16',
         '매장 CRUD, 이미지 갤러리, 다국어 번역 캐시',
         '✅', ''),
        ('매장 정보', 'Phase 1', 'P5',
         'StoreInfo 하드코딩 제거 + StoreService 실연동 + PwFooter + dead code 정리',
         '✅', '백엔드 schema 부재(category/holidays) → P8c 별도 작업'),
        ('매장 정보', '남은', 'P8c (별도 작업 칩)',
         'facilities 스키마 category/holidays/detailAddress 컬럼 추가 또는 UI 정리',
         '⬜', '결정 필요'),

        # ───────── 결제·구독 ─────────
        ('결제·구독', '이전 완료', 'PR #39, #41, #43~46, #56',
         'billing API 전체(cards/subscriptions/payments/refund/extend), PG provider 추상화',
         '✅', '백엔드 sim 모드 — 토스 키 활성화 시 운영'),
        ('결제·구독', 'Phase 1', 'P7',
         'provider 카드 평문저장 제거(PCI) + BillingService + Payment/Subscription/ServiceRequest 실연동',
         '✅', 'admin Payments는 이미 정합'),
        ('결제·구독', '남은', '토스 키 활성화',
         '토스페이먼츠 가맹점 심사 통과 후 PG_PROVIDER=toss + 실 결제 검증',
         '⬜', '외부(심사 1~2주)'),

        # ───────── 쿠폰·스탬프 ─────────
        ('쿠폰·스탬프', '이전 완료', 'PR #6, #18, #22, #29',
         '쿠폰 발행/사용/만료, 스탬프 적립/리워드, 정책 설정',
         '✅', ''),
        ('쿠폰·스탬프', 'Phase 1', 'P9',
         'provider Coupons·Stamps mock→실연동 + mobile 쿠폰 사용 silent error 수정',
         '⬜', ''),
        ('쿠폰·스탬프', 'Phase 1', 'P22',
         '회원 QR (URL 토큰 인코딩) + provider 스캔/코드입력 → 적립/사용 + 친구초대 QR/링크',
         '⬜', 'P9 이후 병행'),

        # ───────── 채팅 ─────────
        ('채팅', '이전 완료', 'PR #16, #21, #142',
         '1:1 채팅 rooms/messages, SSE 스트림, 신고/차단',
         '✅', ''),
        ('채팅', 'Phase 1', 'P8 (◑)',
         'provider CustomerChat 실연동 + admin ChatMonitor + BE /api/chat/reports + mobile SSE 재연결',
         '◑', '⚠ app.py 재시작 후 활성'),
        ('채팅', '남은', 'P8b',
         '채팅 자동 번역(번역 캐시 스키마 + 뷰어별 언어 + translator 연결)',
         '⬜', 'DeepL/Google Translate 키 확보 후'),

        # ───────── 알림 ─────────
        ('알림 (Inbox + Push)', '이전 완료', 'PR #38, #44, #50',
         'Inbox·Announcements, FCM 푸시, APNs(HTTP/2 JWT), 알림 선호 카테고리',
         '✅', ''),
        ('알림 (Inbox + Push)', 'Phase 1', 'P10',
         'provider Notifications 실연동 + mobile 알림 라우팅 + 알림 탭 보강',
         '⬜', ''),

        # ───────── WiFi ─────────
        ('WiFi (1회 연결)', '이전 완료', 'PR #11, #49',
         'wifi_profiles 기본, native plugin(iOS/Android) 자동 가입 요청, BLE 핸드셰이크',
         '✅', '1회 연결만, 로밍 미지원'),
        ('WiFi 로밍 (B 풀스코프)', 'Phase 1', 'P14~P19',
         'wifi_profiles 확장 + beacon_wifi·units·grant·devices + admin/provider 등록 + mobile 클라이언트 + .mobileconfig 다건 + managed/units (flag)',
         '⬜', '6월 말~7월 중'),

        # ───────── 비콘 ─────────
        ('비콘 (인벤토리/claim/모니터)', '이전 완료', 'PR #37, #38',
         'admin 인벤토리 CSV 입고, provider claim, 배터리 모니터링, 펌웨어 표시',
         '✅', ''),
        ('비콘', 'Phase 1', 'P15 (병합)',
         'beacons.role(wifi/cashier) 추가 + admin·provider UI',
         '⬜', ''),

        # ───────── i18n ─────────
        ('i18n (12언어)', '이전 완료', 'BE translations API + admin CRUD',
         '백엔드 translations 테이블, DeepL 통합, 12개 supportedLocales, admin CRUD',
         '✅', ''),
        ('i18n', 'Phase 1', 'P2',
         'mobile I18nService DB 통일 + 전 화면 t() 전환 + ko 시드 550 + DeepL 일괄번역 스크립트',
         '✅', '11개 언어 번역 = DeepL 키 확보 후 batch 실행'),
        ('i18n', '남은', 'DeepL 번역 batch',
         'translations DB의 ko 키 → 11개 언어 자동 번역(약 22 × 549건)',
         '⬜', 'DeepL API 키 활성화 후'),

        # ───────── 정책/약관 ─────────
        ('정책/약관', '이전 완료', '8종 KIND 등록',
         'terms, privacy, location, marketing, push, camera, storage, third_party, age14 DB 등록',
         '✅', ''),
        ('정책/약관', 'Phase 1', 'P13',
         '환불·청소년·쿠키 정책 3종 BE/admin/노출 추가 + policy_view 언어 정합',
         '⬜', ''),

        # ───────── 디자인 시스템 ─────────
        ('디자인 시스템', '이전 완료', '3콘솔 디자인 정합',
         'mobile dark + 보라, provider-web dark + 녹색, admin-web dark + 블루 — 3콘솔 디자인 토큰 통일',
         '✅', ''),
        ('디자인 시스템', 'Phase 1', 'P1·P3',
         'mobile 테마 단일화 + 시스템 폰트 + Pw* 위젯 / 웹 alert·confirm 33곳 → 공용 모달',
         '✅', ''),

        # ───────── 슈퍼어드민 ─────────
        ('슈퍼어드민 (admin-web)', '이전 완료', 'PR #36~#39, #142',
         'Login + Dashboard + Beacons + Approvals + Battery + Announcements + Payments + Subscriptions + Policies + Reports + Translations + Audit',
         '✅', ''),
        ('슈퍼어드민', 'Phase 1', 'P11·P20',
         'Dashboard 가짜데이터 제거 + StaffMonitor·CouponStats placeholder 해소 + app-versions UI(BE 완성)',
         '⬜', ''),

        # ───────── 직원 관리 ─────────
        ('직원 관리', '이전 완료', '백엔드 staff API',
         '직원 초대/권한/관리 백엔드',
         '✅', ''),
        ('직원 관리', 'Phase 1', 'P11',
         'provider StaffManagement 실연동 + admin StaffMonitor 실연동',
         '⬜', ''),

        # ───────── 심의 / 출시 ─────────
        ('심의 직격 (mobile)', 'Phase 1', 'P12',
         'consent placeholder 제거 + 동의 로드 실패 복구 + settings dev 정보 제거 + 앱 버전 동적화',
         '⬜', '심의 reject 방지'),
        ('심의 메타 자산', 'Phase 1', 'P21',
         'PrivacyInfo.xcprivacy + Bundle ID 확정 + Android Photo Picker + 계정삭제 웹 URL',
         '⬜', 'iOS/Android 심사 필수'),
        ('OCR 허위 제거', 'Phase 1', 'P6',
         'WifiSettings·ServiceRequest runOcrMock 제거 → 정직한 수동입력 UI',
         '✅', '심의 reject 방지'),
        ('회사 정보 (footer)', 'Phase 1', 'P5 (포함)',
         'Settings 푸터 → PwFooter (CompanyInfoService → 트리거소프트 실 법인정보)',
         '✅', '법인 등기 완료'),

        # ───────── 운영 인프라 ─────────
        ('운영 인프라', '이전 완료', 'PR #35, #41, #51',
         'SECRET_KEY/AES_KEY ENV + CORS 화이트리스트 + rate-limit + gunicorn/wsgi + Sentry + PostgreSQL 어댑터',
         '✅', '실 호스팅은 외부 서비스 활성화 후'),

        # ───────── 후속 단계 ─────────
        ('Phase 2~4 테스트', '남은', 'W7 테스트 시드',
         '6 페르소나(외국인/한국인/소규모/중대형/직원/슈퍼어드민) 테스트 데이터 시드 + 시나리오 검증',
         '⬜', '7월 중순~하순'),
        ('Phase 5 제출', '남은', '스토어 제출',
         'iOS/Android 빌드 + 스토어 메타데이터 + 스크린샷 + 심의 제출',
         '⬜', '7월 하순~8월 초'),
    ]
